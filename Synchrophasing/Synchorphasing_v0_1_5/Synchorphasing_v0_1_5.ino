...
int j = 0;
int n = 0; // both defined outside of loop to ensure that they only run the once

void loop() {
    
    int i = 0; // gonna use i a lot so may as well initialise now  
    // number of propellers needed to be corrected, if motor 1 is off SetRPM we can increase this one first. 
    ...
    Serial.print("RPM ");
    Serial.print(props[0].rpm);
    Serial.print("  ");
    Serial.print(props[1].rpm);
    Serial.print("  ");
    Serial.print(props[2].rpm);
    Serial.print("  ");
    Serial.println(props[3].rpm);
    Serial.print("PWM ");
    ...
    // NOw this is set up we can run the while loop to create another loop
    ...
    while(PWMVal[0] > 1000) { // Make sure theyre actually running
        ...
        // run control for MOTOR 1 - Just a side thing to ensure getting the closest value.
            ...
            n = 1;
            SETRPM = props[0].rpm;
            Serial.print(" WITHIN ACCEPTABLE RANGE, NEW SETRPM IS ");
        ...
        for(i = n; i < Pr;  i++) {
            ...
            */
            ...
            while((props[i].rpm > rpmtol_above && props[i].rpm < rpmtol_below)) {
                ...
                Serial.println(" RPM MATCH RUNNING"); 
                Serial.println("__________________");
