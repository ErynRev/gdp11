...
constexpr uint32_t RPM_STABLE_TIME_MS = 300;
...
float RPM_STABLE_BAND[NUM_PAIRS] = {100.0f, 100.0f};
...
void handleSerial() {
  ...
  if (line == "?") {
    Serial.print("TARGET_PHASE_DEG = ");
    Serial.println(TARGET_PHASE_DEG, 3);

    Serial.print("K_P_phi[0] = ");
    Serial.println(K_P_phi[0], 6);
    Serial.print("K_I_phi[0] = ");
    Serial.println(K_I_phi[0], 6);
    Serial.print("K_D_phi[0] = ");
    Serial.println(K_D_phi[0], 6);

    Serial.print("K_P_phi[1] = ");
    Serial.println(K_P_phi[1], 6);
    Serial.print("K_I_phi[1] = ");
    Serial.println(K_I_phi[1], 6);
    Serial.print("K_D_phi[1] = ");
    Serial.println(K_D_phi[1], 6);

    return;
  }
  if (line.startsWith("tp") || line.startsWith("TP")) {
    String valStr = line.substring(2);
    valStr.trim();

    float v = valStr.toFloat();
    TARGET_PHASE_DEG = v;

    Serial.print("New TARGET_PHASE_DEG = ");
    Serial.println(TARGET_PHASE_DEG, 3);
    return;
}
...
void setup() {
  ...
  K_P_phi[0] = 0.2;
  K_I_phi[0] = 0.05;   // preserved from your original code
  K_D_phi[0] = 0.1;

  
  K_P_phi[1] = 0.2;
  K_I_phi[1] = 0.05;   // preserved from your original code
  K_D_phi[1] = 0.1;
...
void loop() {
  ...
  for (uint8_t i = 0; i < NUM_PAIRS; i++) {
    ...
    // -------- PHASE MODE --------
      ...
      throttle_phase = constrain(throttle_phase, throttle_us - 20, throttle_us + 20);
